...
# Main tabs
# ===========================================================================
(_tab_contract, _tab_paper, _tab_live, _tab_activity, _tab_accuracy,
    _tab_analysis) = st.tabs([
    "Contract Data", "Paper Trade Data", "Live Trade Data", "Activity",
...
# ===========================================================================
# TAB 6 — Analysis (bot persistence + performance + calibration audit)
# ===========================================================================
# Surfaces the data the bot persists per purchase so it can be used to
# improve the model.  Each section answers ONE diagnostic question.
# All queries are read-only and cached for _CACHE_TTL_SHORT.

with _tab_analysis:
    _show_refresh_messages()

    st.markdown("## Bot Analysis & Persistence Audit")
    st.caption(
        "Verifies the data the bot is saving per purchase, and surfaces "
        "performance + calibration metrics for improving the model.  "
        "All queries are read-only.  See HANDOFF \xa72.10 for the underlying "
        "persistence flow."
    )
    ...
    # Lookback control
    # ------------------------------------------------------------------
    _an_c1, _an_c2 = st.columns([1, 4])
    with _an_c1:
        _analysis_days = st.selectbox(
            "Lookback",
            options=[7, 14, 30, 60, 90, 180],
            index=2,    # default 30 days
            key="analysis_lookback_days",
            help="Window for closed-position queries (P&L, calibration, etc.)",
        )

    # ------------------------------------------------------------------
    # Section 1 — Pipeline coverage (signal -> order -> position)
    # ------------------------------------------------------------------
    st.divider()
    st.markdown("### 1. Pipeline coverage")
    st.caption(
        "Every LIVE_BUY signal should produce an order; every successful "
        "order should produce a position.  Large gaps mean writes are "
        "dropping somewhere in the chain."
    )

    @st.cache_data(ttl=_CACHE_TTL_SHORT)
    def _load_pipeline_counts():
        conn = _connect_db()
        try:
            sig = conn.execute(
            ...
            return sig, ord_n, pos
        finally:
            conn.close()

    try:
        _sig_n, _ord_n, _pos_n = _load_pipeline_counts()
        _pc1, _pc2, _pc3 = st.columns(3)
        with _pc1:
            st.metric("LIVE_BUY signals today", _sig_n)
        with _pc2:
            st.metric("Orders placed today", _ord_n,
                        delta=(_ord_n - _sig_n) if _sig_n else None,
                        delta_color="off")
        with _pc3:
            st.metric("Live positions opened today", _pos_n,
                        delta=(_pos_n - _ord_n) if _ord_n else None,
                        delta_color="off")
        if _sig_n > 0 and _ord_n == 0:
            st.error(
                "WARNING: LIVE_BUY signals exist but ZERO orders placed today. "
                "Check execute_signal logs for failures."
            )
        elif _ord_n > 0 and _pos_n == 0:
            st.warning(
                "Orders placed but no positions opened.  Either all orders "
                "errored, or position row inserts are failing.  Check the "
                "live_predictor_orders.error column."
            )
    except Exception as _e:
        st.warning(f"Could not load pipeline counts: {_e}")

    # ------------------------------------------------------------------
    # Section 2 — Aggregate performance scorecard
    # ------------------------------------------------------------------
    st.divider()
    st.markdown("### 2. Weekly performance scorecard")
    st.caption(
        "Real-money P&L on closed LIVE positions, grouped by ISO week.  "
        "win_pct is bin-by-bin (not bankroll-weighted); pnl_total is net "
        "of fees (uses pnl_net when available, else gross pnl)."
    )

    @st.cache_data(ttl=_CACHE_TTL_SHORT)
    def _load_weekly_perf():
        conn = _connect_db()
        try:
            return pd.read_sql(
                """
    ...
    _wp = _load_weekly_perf()
    if _wp.empty:
        st.info("No closed live positions yet.")
    else:
        _wp_sorted = _wp.iloc[::-1].copy()   # asc for chart
        fig_w = go.Figure()
        fig_w.add_trace(go.Bar(
            x=_wp_sorted["week"], y=_wp_sorted["pnl_total"],
            marker_color=[
                "#2ecc71" if v >= 0 else "#e74c3c"
                for v in _wp_sorted["pnl_total"]
            ],
            name="P&L ($)",
            text=[f"${v:+.0f}" for v in _wp_sorted["pnl_total"]],
            textposition="outside",
        ))
        fig_w.update_layout(
            yaxis_title="P&L ($)", xaxis_title="Week",
            height=260, margin=dict(t=20, b=40),
        )
        fig_w.add_hline(y=0, line_color="gray", opacity=0.5)
        st.plotly_chart(fig_w, width="stretch")

        _wp_show = _wp.copy()
        _wp_show.columns = ["Week", "Closed", "Wins", "Win %",
                              "Total Staked", "P&L", "ROI %"]
        st.dataframe(_wp_show, hide_index=True, width="stretch",
                      height=min(60 + len(_wp_show) * 35, 320))

        _wp_tot1, _wp_tot2, _wp_tot3, _wp_tot4 = st.columns(4)
        with _wp_tot1:
            st.metric("Total closed", int(_wp["n_closed"].sum()))
        with _wp_tot2:
            _wins = int(_wp["wins"].sum())
            _n = int(_wp["n_closed"].sum())
            _wr = (100.0 * _wins / _n) if _n else 0
            st.metric("Lifetime win %", f"{_wr:.1f}%")
        with _wp_tot3:
            st.metric("Total staked", f"${_wp['total_staked'].sum():.2f}")
        with _wp_tot4:
            _pnl_sum = float(_wp["pnl_total"].sum())
            st.metric("Lifetime P&L", f"${_pnl_sum:+.2f}",
                        delta_color=("normal" if _pnl_sum >= 0 else "inverse"))

    # ------------------------------------------------------------------
    # Section 3 — Recent closed positions with realized P&L
    # ------------------------------------------------------------------
    st.divider()
    st.markdown("### 3. Recent closed positions (realized P&L)")
    st.caption(
        f"All closed LIVE positions in the past {_analysis_days} days.  "
        "exit_price=1.0 means we won, 0.0 means we lost.  "
        "pnl_net is preferred (fees deducted); falls back to gross pnl."
    )

    @st.cache_data(ttl=_CACHE_TTL_SHORT)
    def _load_closed_positions(days: int):
        conn = _connect_db()
        try:
            return pd.read_sql(
                f"""
    ...
    _cp = _load_closed_positions(_analysis_days)
    if _cp.empty:
        st.info(f"No closed live positions in the last {_analysis_days} days.")
    else:
        _cp_show = _cp.copy()
        _cp_show["date"] = _cp_show["date"].apply(_fmt_date_mmddyyyy)

        def _color_pnl_an(val):
            if pd.isna(val):
                return ""
        ...
        _cp_show.columns = ["Date", "City", "Side", "Stake", "Entry",
                              "Exit", "P&L", "Return %", "Exit Reason",
                              "Exit Time"]
        st.dataframe(
            _cp_show.style.map(_color_pnl_an, subset=["P&L", "Return %"]).format({
                "Stake":    "${:.2f}",
                "P&L":      lambda v: f"${v:+,.2f}" if pd.notna(v) else "",
                "Return %": lambda v: f"{v:+.1f}%" if pd.notna(v) else "",
            }),
            hide_index=True, width="stretch",
            height=min(60 + len(_cp_show) * 32, 480),
        )

    # ------------------------------------------------------------------
    # Section 4 — Model calibration (our_prob vs realized win rate)
    # ------------------------------------------------------------------
    st.divider()
    st.markdown("### 4. Model calibration")
    st.caption(
        "Bucket closed positions by the model's confidence at buy time "
        "(our_prob) and check the realized win rate.  If our model is "
        "well-calibrated, avg_model_p should approximately equal "
        "actual_win_rate within each bucket.  Persistent gap > 10pp = "
        "miscalibration to investigate."
    )

    @st.cache_data(ttl=_CACHE_TTL_SHORT)
    def _load_calibration(days: int):
        conn = _connect_db()
        try:
            return pd.read_sql(
                f"""
                WITH joined AS (
                    ...
                    WHERE COALESCE(p.is_paper, 0) = 0
                      ...
                      AND p.date >= date('now', '-{int(days)} days')
                )
                SELECT
                    CASE
    ...
    _cal = _load_calibration(_analysis_days)
    if _cal.empty:
        st.info("Need at least one closed position with a matching LIVE_BUY "
                "signal to compute calibration.")
    else:
        _total_n = int(_cal["n"].sum())
        if _total_n < 30:
            st.warning(
                f"Only {_total_n} closed position(s) total in window.  "
                f"Calibration is anecdotal until ~30+ per bucket."
            )

        fig_c = go.Figure()
        fig_c.add_trace(go.Scatter(
            x=_cal["avg_model_p"], y=_cal["actual_win_rate"],
            mode="markers+text",
            marker=dict(
                size=[max(10, min(30, n)) for n in _cal["n"]],
                color="#3498db",
            ),
            text=[f"n={n}" for n in _cal["n"]],
            textposition="top center",
            name="bucket",
        ))
        fig_c.add_trace(go.Scatter(
        ...
        fig_c.update_layout(
            xaxis_title="Model probability (our_prob)",
        ...
        _cal_show = _cal.copy()
        _cal_show.columns = ["Conf bucket", "N", "Avg model_p",
                                "Actual win rate", "Gap (actual − model)"]

        def _color_gap(val):
            if pd.isna(val): return ""
            mag = abs(val)
            if mag >= 0.10: return "color: #e74c3c; font-weight: bold"
            if mag >= 0.05: return "color: #f39c12"
            return "color: #2ecc71"

        st.dataframe(
            _cal_show.style.map(_color_gap, subset=["Gap (actual − model)"]),
            hide_index=True, width="stretch",
            height=min(60 + len(_cal_show) * 35, 280),
        )

    # ------------------------------------------------------------------
    # Section 5 — Model context preserved (signal join check)
    # ------------------------------------------------------------------
    st.divider()
    st.markdown("### 5. Filled positions with preserved model context")
    st.caption(
        "Recent filled live positions joined to the LIVE_BUY signal that "
        "produced them.  Empty model columns = the signal row didn't "
        "persist that field (data-loss bug to fix)."
    )

    @st.cache_data(ttl=_CACHE_TTL_SHORT)
    def _load_filled_with_context(days: int):
        conn = _connect_db()
        try:
            return pd.read_sql(
                f"""
    ...
    _fc = _load_filled_with_context(_analysis_days)
    if _fc.empty:
        st.info("No filled live positions in window.")
    else:
        _missing_cols = [c for c in ("model_p", "mu_c", "sigma_c",
                                        "fc_high_c", "dq_flag")
                            if _fc[c].isna().all()]
        if _missing_cols:
            st.error(
                f"Data-loss warning: columns always-NULL in this window: "
                f"{_missing_cols}.  Either the signal row isn't writing "
                f"these fields, or the join is missing."
            )
        elif _fc[["model_p", "mu_c"]].isna().any().any():
            _n_missing = int(_fc[["model_p", "mu_c"]].isna().any(axis=1).sum())
            st.warning(
                f"{_n_missing} of {len(_fc)} positions have NO matching "
                f"signal row — order was placed without a LIVE_BUY signal "
                f"in the same day's scan (manual trade, or signal was "
                f"deleted)."
            )
        _fc_show = _fc.copy()
        _fc_show["date"] = _fc_show["date"].apply(_fmt_date_mmddyyyy)
        _fc_show.columns = ["Pos ID", "City", "Date", "Filled $", "Entry",
                              "model_p", "mkt_p", "Edge", "μ (C)",
                              "σ (C)", "FC High (C)", "DQ Flag",
                              "Entry Time"]
        st.dataframe(_fc_show, hide_index=True, width="stretch",
                      height=min(60 + len(_fc_show) * 32, 440))

    # ------------------------------------------------------------------
    # Section 6 — Event resolutions seen by the bot
    # ------------------------------------------------------------------
    st.divider()
    st.markdown("### 6. Event resolutions seen by the bot")
    st.caption(
        "Markets the resolution monitor (_settle_resolved_positions) "
        "has observed settle on Gamma.  Empty for a recent date range "
        "often means resolutions haven't happened yet — not necessarily "
        "a bug."
    )

    @st.cache_data(ttl=_CACHE_TTL_SHORT)
    def _load_resolutions(days: int):
        conn = _connect_db()
        try:
            return pd.read_sql(
                f"""
                SELECT
                    date, city,
                    (ROUND(winning_range_low,0) || '-' ||
                     ROUND(winning_range_high,0)) AS winning_bin,
    ...
    _rs = _load_resolutions(_analysis_days)
    if _rs.empty:
        st.caption(f"No resolutions recorded in the last {_analysis_days} days.")
    else:
        _rs_show = _rs.copy()
        _rs_show["date"] = _rs_show["date"].apply(_fmt_date_mmddyyyy)
        _rs_show.columns = ["Event Date", "City", "Winning Bin",
                              "YES Settle Px", "Resolved At"]
        st.dataframe(_rs_show, hide_index=True, width="stretch",
                      height=min(60 + len(_rs_show) * 32, 400))

    # ------------------------------------------------------------------
    # Section 7 — Stuck positions (past event date, still open)
    # ------------------------------------------------------------------
    st.divider()
    st.markdown("### 7. Stuck-position alert")
    st.caption(
        "Live positions whose event_date has already passed but the bot "
        "hasn't marked closed.  Usually means the resolution monitor "
        "(`_settle_resolved_positions`) isn't firing, or Gamma still "
        "shows the market open (delayed resolution)."
    )

    @st.cache_data(ttl=_CACHE_TTL_SHORT)
    def _load_stuck_positions():
        conn = _connect_db()
        try:
            return pd.read_sql(
                """
    ...
    _stuck = _load_stuck_positions()
    if _stuck.empty:
        st.success("No stuck positions — every past-date live "
                     "position is closed.  Resolution monitor is healthy.")
    else:
        st.warning(
            f"{len(_stuck)} live position(s) past event date, still open.  "
            f"Investigate whether `_settle_resolved_positions` is firing "
            f"OR Gamma shows the market still open (delayed resolution)."
        )
        _stuck_show = _stuck.copy()
        _stuck_show["date"] = _stuck_show["date"].apply(_fmt_date_mmddyyyy)
        _stuck_show.columns = ["Pos ID", "City", "Event Date", "Side",
                                 "Stake", "Entry", "Status", "Fill Status",
                                 "Entry Time", "Days Past Event"]
        st.dataframe(_stuck_show, hide_index=True, width="stretch",
                      height=min(60 + len(_stuck_show) * 32, 320))

    # ------------------------------------------------------------------
    # Refresh control
    # ------------------------------------------------------------------
    st.divider()
    _an_r1, _an_r2, _an_r3 = st.columns([2, 1, 2])
    with _an_r2:
        if st.button("Refresh Data", key="refresh_analysis", type="primary"):
            _do_refresh("refresh_analysis")
